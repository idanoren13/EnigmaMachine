...
public class InitCode {
    ...
    public List<Integer> createSelectedRotorsList(String selectedRotors) throws NumberFormatException, InvalidRotorException {
        ...
        if (new HashSet<>(listRotors).size() < listRotors.size()) {
    ...
    public String pickRandomRotors(int rotorsCount, int totalRotors) {
        ...
        IntStream.rangeClosed(1, totalRotors).forEach(allIDs::add); // List from 1 to totalRotors: [1, 2, 3, ..., totalRotors]
    ...
    public Reflector.ReflectorID getReflectorID(String reflector) throws InvalidReflectorException, IllegalArgumentException {
        int reflectorID = convertRomanNumeralsLettersToInt(reflector);
    ...
    private int convertRomanNumeralsLettersToInt(String roman) throws IllegalArgumentException {
        int result = 0;
