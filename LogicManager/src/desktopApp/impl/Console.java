...
public class Console implements Input {
    ...
    public void readMachineFromXMLFile(InputStream stream) throws InvalidMachineException, JAXBException, InvalidRotorException, IOException, InvalidABCException, UnknownSourceException, InvalidReflectorException {
        File file = new File(stream.toString());
        CTEEnigma cteEnigma = (CTEEnigma) JAXBContext.newInstance(CTEEnigma.class).createUnmarshaller().unmarshal(file);
        this.engine = new EnigmaMachineFromXML().getEnigmaEngine(cteEnigma);
//        createDTOFromXMLSource(stream);
    }
